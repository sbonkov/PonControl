...
<form method="post" action="set_onu_port_down.php?olt=<?php echo $ip; ?>&iface=<?php echo $iface; ?>&port=<?php echo $port; ?>&mac=<?php echo $mac; ?>">
<input name="add" type="submit" id="add" value="Погасить пиздюка" style="padding: 2px;">
</form>

</div><div style="white-space: nowrap; padding-left: 10px; float: left; position: relative">
<form method="post" action="set_onu_port_up.php?olt=<?php echo $ip; ?>&iface=<?php echo $iface; ?>&port=<?php echo $port; ?>&mac=<?php echo $mac; ?>">
<input name="add" type="submit" id="add" value="Быть Няшкой )" style="padding: 2px;">
</form>
